...
async function getAllRecipes(req, res, next) {
	...
	if (req.query.name) {
		try {
			...
			let promiseApi = await axios.get(`${BASE_URL}complexSearch?query=${name}&apiKey=${API_KEY_1}&${URL_FLAGS}`)
		...
		try {
			...
			let promiseApi = await axios.get(`${BASE_URL}complexSearch?&apiKey=${API_KEY_1}&${URL_FLAGS}`)
			if (!promiseApi.data.results.length && !recipesDB.length) {
				throw new Error ('ERROR 500: Your search has 0 results');
			}
			let apiResponse = [];
...
async function getRecipeById(req, res, next) {
	...
	if (/^[^A-Za-z]+$/i.test(idReceta)) {
		let promiseApi;
		try {
			promiseApi = await axios.get(`${BASE_URL}${idReceta}/information?apiKey=${API_KEY_1}`);
